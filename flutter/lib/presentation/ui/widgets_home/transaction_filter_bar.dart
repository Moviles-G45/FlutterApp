...
class _TransactionFilterBarState extends State<TransactionFilterBar> {
  ...
  Future<void> _selectDate(BuildContext context, bool isStart) async {
    ...
    if (!isStart && _startDate != null) {
      firstDate = _startDate!; // La fecha de fin no puede ser anterior a la de inicio
    }
    ...
    try {
        ...
        setState(() {
          if (isStart) {
            ...
            if (_endDate != null && _endDate!.isBefore(_startDate!)) {
              ...
              _showMessage(
                  context,
                  "End date reset as it was earlier than start date.",
                  Colors.redAccent);
            }
          } else {
            _endDate = picked;
      ...
      _showMessage(
          context, "Select a date before today or today.", Colors.redAccent);
  ...
  void _applyFilter() {
    ...
    try {
      ...
      if (homeVM.isOffline) {
        _showMessage(context, "No internet connection. Please check your network.", Colors.redAccent);
        return;
      }

      if (end.isBefore(start)) {
        _showMessage(context, "End date cannot be earlier than start date.",
            Colors.redAccent);
        return;
      }
      ...
      _showMessage(context, "Error applying filter. Please try again.",
          Colors.redAccent);
